...
public class MapComponent extends JPanel implements TrafficSimObserver {
	...
	private void drawRoads(Graphics g) {
		for (Road r : _map.getRoads()) {
			...
			int x1 = r.getOrigin().getxCoor();
			int y1 = r.getOrigin().getyCoor();
			int x2 = r.getDestiny().getxCoor();
			int y2 = r.getDestiny().getyCoor();

			// choose a color for the arrow depending on the traffic light of the road
			Color arrowColor = _RED_LIGHT_COLOR;
			int idx = r.getDestiny().getGreenLight();
			if (idx != -1 && r.equals(r.getDestiny().getInRoads().get(idx))) {
			...
			int roadColorValue = 200 - (int) (200.0 * Math.min(1.0, (double) r.getTotalContamination() / (1.0 + (double) r.getContAlarm())));
	...
	public void onError(String err) {
		JOptionPane.showMessageDialog(this, err, "Error Map Panel", JOptionPane.ERROR_MESSAGE);
	}
